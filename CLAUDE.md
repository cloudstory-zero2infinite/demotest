...
## Project Overview
ZeroTo1 GRC (codebase/package name) is a Governance, Risk & Compliance SaaS platform with multi-tenant support. It provides security teams and CXOs with tools to manage programs, internal controls, assets, policies, vulnerabilities, compliance, and contacts. The user-facing product name in the UI is **Zero to Infinite — Unified Cyber Platform** (header tagline / onboarding); internal identifiers, buckets, and package names still use `grc`/`ZeroTo1 GRC`.
...
- **Mapper Visualizer** (`components/governance/MapperVisualizerView.tsx`): renders the Neo4j knowledge graph using `@xyflow/react` with a `d3-force` layout. Master + its hubs are pinned at fixed angles; spokes are pulled to the outward side of each hub by a custom directional force. Every node has a single hidden centre handle (so straight edges appear to emerge from the node edge). Triggered from the Policy tab via `MapperRunModal.tsx`. Tree edges use `DEFINES/HAS_CHILD/MAPS_TO`; `COVERS` is rendered as a cross-link overlay. `MapperRunModal` lets you **re-select the master policy** ("Change master policy") and run even when a master is already set, in addition to the `needs_master` flow.

- **Dashboard chart conventions**: new dashboard cards should use the shared `components/dashboard/ChartCard.tsx` shell (consistent header + expand-to-modal) and the tokens in `components/dashboard/chartTheme.ts` (single palette, shared Recharts `Tooltip` style, status/category colors) rather than re-declaring inline styles. A true sunburst is done with two nested Recharts `<Pie>` rings sharing one total (see `ControlsCoverageCard.tsx`); there is no sunburst lib.

### Backend (`/server/`)
- **Express.js** (Node.js, ESM modules — `"type": "module"`)
- Entry: `server/src/index.js`
- Routes: `server/src/routes/` — one file per domain (program, controls, assets, asset-types, asset-custom-fields, custom-fields, policies, vulnerabilities, compliance, scoring, contacts, activity, org, org-settings, org-contacts, email-templates, feedback, capabilities, control-registry, mapper). Note: `scoring` is mounted under `/api/compliance` alongside the compliance router; `mapper` is a thin proxy that forwards `/api/mapper/*` to the ai-agent at `AI_AGENT_URL`
...
- Email: **Resend** (`resend` npm package) is used only by the in-app feedback form (`routes/feedback.js`). All other transactional email — currently the policy expiry reminders — goes through **nodemailer over SMTP** in `server/src/lib/email.js` (the same SMTP server configured for Supabase Auth, sender `noreply@cloudstory.ind.in`). Config: `SMTP_HOST/PORT/USER/PASS/FROM` in `server/.env`; if SMTP is unset the helper no-ops. `email.js` exports `sendEmail`, `policyDeepLink`, `sendPolicyExpiryReminder`, and `DEFAULT_EXPIRY_TEMPLATE`.
- File uploads: **Multer** with 50MB limit
- Scheduled jobs: `server/src/jobs/policy-expiry.js` — cron every 6 hours (`0 */6 * * *`) runs **two** functions: `checkAllExpiredPolicies()` (flips `approved` policies whose `refresh_date` has passed to the `overdue` status, retaining the date, and logs + notifies) and `sendExpiryReminders()` (emails escalating reminders 14 / 7 / 1 days before due). Reminders are idempotent per approval cycle via `reminder_14d/7d/1d_sent_at` columns (reset to NULL on each approval), so the 6h cadence sends each reminder at most once. Recipients are deduped from the policy's approver/reviewer (`policy_approvals.approver_email`), the org's tenant admins (`org_onboarding` role admin/tenant_admin), and the policy owner. The email subject/body come from the org's selected template (see Email templates below), falling back to `DEFAULT_EXPIRY_TEMPLATE`.
...
- Submodules:
  ...
  - `fwcr_agent/` (Fw-ControlRegistry Agent) — **deterministic, no LLM**. Rebuilds an org's standard `control_registry` rows from the SCF framework selection in `organizations.needed_framework`. Endpoints: `POST /fwcr/recompute-preview` (returns the add/update/delete/keep diff as a dry run) and `POST /fwcr/recompute` (applies). Driven from the Settings → Organisation tab's "Recompute Control Registry and Save" button via the Express proxy `server/src/routes/fwcr.js`. **Protection rule:** rows where `ctl_status != 'NotEnforced'` OR `evidence_metadata` is non-empty survive even when their framework is deselected — `ctl_ref_fw` retains the original names as a paper trail. New `control_registry` rows are composed as `ctl_name = "<SCF#>-<control_name>"` (e.g. `GOV-01.1-Steering Committee & Program Oversight`), `ctl_id = <SCF#>`, `ctl_type = 'Standard'` (capital S — matches the CHECK constraint), `scf_control_id = <SCF#>` (stable agent-ownership key).
  - **NN baseline + recompute orchestration.** The Settings recompute button is **always available** and orchestrates two independent things in the frontend (`OrgSettingsTab.tsx`): (1) the fwcr agent recompute above (framework standards), and (2) re-seeding the **Non-Negotiable (NN) baseline** via `POST /api/controls/seed-nn` → `seed_nn_controls_for_org`. NN is shown in the picker as a locked, non-removable "Non-Negotiables" chip and is **not** stored in `needed_framework`. The dry-run preview combines both (framework add/update/delete counts + `GET /api/controls/nn-preview` "NN to (re)add" count). The fwcr agent itself still never touches NN rows (`scf_control_id IS NULL`); NN and frameworks are orthogonal.
...
- Server also needs: `SUPABASE_URL`, `SUPABASE_SERVICE_ROLE_KEY` (for `supabaseAdmin`), `RESEND_API_KEY` (feedback email only), `SMTP_HOST`/`SMTP_PORT`/`SMTP_USER`/`SMTP_PASS`/`SMTP_FROM` (policy expiry reminder email via nodemailer; `SMTP_FROM` defaults to `noreply@cloudstory.ind.in`), `FRONTEND_URL` (CORS + the email deep-link), `AI_AGENT_URL` (used by the `/api/mapper/*` proxy)
...
- **`program` table (Program Tracker).** Each task has `task_code` (human ID `TSK-<2-char org prefix>-<seq>`, e.g. `TSK-AB-001`, generated per-org in `server/src/routes/program.js`) and `parent_id` (self-FK, `ON DELETE SET NULL`). Parent/child is **two levels only** — the backend rejects nesting a task under a child or attaching a task that already has children (`PUT /api/program/:id/parent` attaches/detaches; re-parenting is allowed). CXOs see the full tracker (not a separate view) with a per-session "Show escalated issues only" toggle (default ON, in `sessionStorage`, cleared on logout). Program CRUD is logged to **both** `program_activity_log` (per-task History) **and** the global `all_activity_log` (Activity Logs tab).
...
- **Policy refresh / expiry lifecycle.** `policy_status` CHECK allows `draft | to_review | in_approval | approved | reviewed | overdue`. The due date `refresh_date` is **owned solely by the approve flow** — only `POST /api/policies/:id/approve` sets it (= today + `org_settings.policy_refresh_months`, frozen at approval time so a later interval change only affects future approvals). `PUT /:id` must never touch `refresh_date`; `POST /:id/review` sets `reviewed` with **no** due date (it's an intermediate "ready for approval" state that never expires). Non-approved policies therefore have `refresh_date = NULL` (a one-time backfill enforced this across all tenants). When an approved policy's date lapses, the cron flips it to `overdue` (kept RED in the UI via `effectiveStatus` in `PoliciesView.tsx`, due date retained). `policy_notifications.type` CHECK is `approval_requested | approved | rejected | policy_expired` (the `policy_expired` value was added — earlier inserts silently failed the constraint). Notification clicks deep-link via `Header.tsx` → `App.tsx handleNavigate` → `GovernanceTab`/`PoliciesView`; emails deep-link via `<FRONTEND_URL>/?policyId=<id>` which `App.tsx` reads on load to auto-open the policy. `services/supabase.ts` must expose `markOrgNotificationRead` (its absence previously crashed org-notification clicks).
- **Email templates (Organisation → Templates).** `email_templates` table (per-org: `id, org_id, user_id, name, subject, body, timestamps`) holds reusable templates, CRUD'd via `/api/email-templates` (`routes/email-templates.js`; writes gated to admin/tenant_admin/cxo) and the `OrgTemplatesTab.tsx` sub-tab. `org_settings.policy_expiry_template_id` (FK → `email_templates.id` `ON DELETE SET NULL`) selects which template drives the policy expiry reminders, chosen via a dropdown in `OrgSettingsTab.tsx` (Settings → "Policy expiry email template"; NULL = built-in default). Templates support placeholders `{{policyName}}`, `{{dueDate}}`, `{{policyLink}}`; the cron's `sendExpiryReminders()` resolves the org's template (memoized per run) and renders it into the branded HTML shell in `email.js` (escaping body text, `{{policyLink}}` → a "Review the policy" button appended if absent).
- **Global SCF reference tables (not per-tenant):** `scf_domains` (33 rows), `scf_controls` (~1468 rows), `scf_frameworks` (~250 external framework names like `ISO 27001 2022`, `NIST CSF 2.0`), `scf_control_frameworks` (~30k junction rows mapping each SCF control to the frameworks that claim it, with the original cell content in `mapping_refs` for traceability). All four are **wiped + repopulated** on every internal-tool SCF upload. RLS-enabled but only the service-role key reads them. Source xlsx archived in the `scf-reference` storage bucket.
  - `GET /api/scf/frameworks/controls?framework=<canonical name>` (in `server/src/routes/scf-frameworks.js`) joins `scf_control_frameworks` + `scf_controls` and returns each SCF control with the framework's native reference IDs (`mapping_refs`, which is newline-separated and split into a `refs[]` array). The Compliance tab renders an **accordion with one independently-expandable panel per framework in `needed_framework`** (lazily fetched + cached on first open); each lists every SCF control mapped to that framework alongside its native clause IDs (e.g. ISO `GOV-01.1 → 4.4, 5.1, …`).
- **`control_registry.ctl_ref_fw`** is **JSONB** (array of framework canonical names). Migrated from `TEXT` in 2026-05; legacy single-value entries were wrapped into single-element arrays. The new `control_registry.scf_control_id TEXT` column marks rows **owned by the Fw-ControlRegistry agent** — agent INSERTs/UPDATEs/DELETEs only touch rows where it is set. NN / Custom / pre-feature standard rows (where `scf_control_id IS NULL`) are invisible to the agent and managed manually as before.
  - The `seed_nn_controls_for_org(org_uuid)` SQL function (idempotent + additive: inserts any `nn_control_templates` row whose `ctl_name` isn't already present as `ctl_type='NN'` for the org, never deletes) wraps the template's single `ctl_ref_fw` TEXT value into a one-element JSONB array (`'CIS'` → `["CIS"]`) to match the JSONB column. Many older orgs were seeded before the TEXT→JSONB migration and have an NN gap; they self-heal on the next Settings recompute.

## Knowledge graph: Neo4j Aura
- Used only by the Mapper Agent (the rest of the app talks to Supabase).
- **Tenant root node.** Every tenant's subgraph is rooted at a single `Org` node keyed on `org_id` (`MERGE (o:Org {org_id})`). The writer connects `(:Org)-[:HAS_POLICY]->(:Policy)` for **every** policy in the org, so the whole tenant graph is reachable from, and anchored to, its Org node — this is the structural guard against cross-tenant leakage. The `Org` node is **not** emitted by `read_graph()`, so the visualizer still appears rooted at the master Information Security policy (the org root is invisible above it). Defined in the ontology as the `Org` entity + `HAS_POLICY` relationship (`Org → Policy`, `derivation: from_source`).
- **Both-endpoint org scoping.** Every node carries `org_id`; every Cypher statement uses `MERGE` so re-runs are idempotent. **Every read query in `read_graph()` now filters `{org_id}` on BOTH endpoints of every pattern** (not just the source) — e.g. `(so:SecurityObjective {org_id})-[:MAPS_TO]->(sd:SCFDomain {org_id})` — so a read can never traverse into another tenant's node even if a stray cross-tenant edge ever existed.
- **v2 labels** (SCF-grounded): `Org` (tenant root, hidden from visualizer), `Policy` (with `is_master` property), `SecurityObjective` (LLM-inferred per master, keyed on `org_id` + `master_policy_id` + `name`), `SCFDomain` (mirrored from `public.scf_domains`, materialised per-org and keyed on `org_id` + `scf_id`), and an applied `:OrphanPolicy` label for child policies the LLM didn't link. The legacy `SecurityDomain` / `SecurityFunction` labels are gone — the wipe step in the writer also DETACH-DELETEs any pre-v2 nodes lingering in the per-org subgraph.
- **v2 relationships**: `HAS_POLICY` (Org → every Policy, tenant root), `DEFINES` (master Policy → SecurityObjective), `MAPS_TO` (SecurityObjective → SCFDomain), `HAS_CHILD` (master → child Policy), `COVERS` (child Policy → SCFDomain). The legacy `CONTAINS` edge is gone.
