...
import { AssetCustomField, AssetCustomFieldCreate, AssetCustomFieldUpdate, CustomFieldType } from '../../types';
...
const ColumnManagementModal: React.FC<ColumnManagementModalProps> = ({
  ...
  // Form state
  const [formData, setFormData] = useState<AssetCustomFieldCreate>({
    field_name: '',
    field_label: '',
    field_type: 'text',
    field_options: [],
  ...
  const fetchCustomFields = async () => {
    try {
      setLoading(true);
      setError(null);
      const fields = await SupabaseService.getAssetCustomFields();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingField) {
        ...
        await SupabaseService.updateAssetCustomField(editingField.id, formData);
      } else {
        // Create new field
        await SupabaseService.createAssetCustomField(formData);
      }
  ...
  const handleDelete = async (field: AssetCustomField) => {
    ...
    try {
      setLoading(true);
      await SupabaseService.deleteAssetCustomField(field.id);
      fetchCustomFields();
      onColumnChange();
    } catch (err: any) {
      setError(err.message);
